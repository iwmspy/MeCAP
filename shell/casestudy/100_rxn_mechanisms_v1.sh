...
${EXEC_PAT} -m predict \
  ...
  --checkpoint ${RESL_DIR}/mecap_ref_maa_layer_0/best_model.pt \
  --atom_index_col elec_sites \
  --sdf_name_col name \
  --sdf_mode per_row \
  --sdf_dir ${SAVE_DIR}/confs \
  --sdf_ext .sdf \
  --save_path ${SAVE_DIR}/maa_v1 \
  --output_csv ${SAVE_DIR}/maa_v1/predictions.csv


${EXEC_PAT} -m predict \
  --data ${SAVE_DIR}/unique/100_rxn_mechanisms_no_error_reactants_nuc.csv \
  --checkpoint ${RESL_DIR}/mecap_ref_mca_layer_0/best_model.pt \
  --atom_index_col nuc_sites \
  --sdf_name_col name \
  --sdf_mode per_row \
  --sdf_dir ${SAVE_DIR}/confs \
  --sdf_ext .sdf \
  --save_path ${SAVE_DIR}/mca_v1 \
  --output_csv ${SAVE_DIR}/mca_v1/predictions.csv
